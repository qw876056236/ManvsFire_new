function PreviewManager(camera,roamPath){//var myPreviewManager=new PreviewManager();
    ...
    this.stopFlag=true;
    this.isLoop=true;//如果不进行循环漫游的话，第一行的初始状态就没用了
...
function MakeOneRoamStep(){
    ...
    this.updateParam=function(x1,y1,z1,x2,y2,z2,a1,b1,c1,a2,b2,c2,time){
        ...
        if(this.da>Math.PI)this.da-=2*Math.PI;
        else if(this.da<-Math.PI)this.da+=2*Math.PI;
        if(this.db>Math.PI)this.db-=2*Math.PI;
        else if(this.db<-Math.PI)this.db+=2*Math.PI;
        if(this.dc>Math.PI)this.dc-=2*Math.PI;
        else if(this.dc<-Math.PI)this.dc+=2*Math.PI;
