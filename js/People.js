...
People.prototype.load = function (_this) {
    ...
    function loadBlendMeshWithPromise(_this) {
        ...
        //人物模型比例 female male child_female child_male granny
        var proportion = [0.45,  0.45,  0.05, 0.05, 0]
        surplus = 0;
        ...
        arr2[2] = loadModelPromise(modelURL15);//无walk用idle代替
        arr2[3] = loadModelPromise(modelURL12);//无bend用crawl代替
        arr2[4] = loadModelPromise(modelURL12);
        arr2[5] = loadModelPromise(modelURL15);
        for (num = 0; num < 2; num++) {

            arr3[num] = loadModelPromise(modelURL3);

        }
        arr3[2] = loadModelPromise(modelURL16);
        arr3[3] = loadModelPromise(modelURL13);
        arr3[4] = loadModelPromise(modelURL13);
        arr3[5] = loadModelPromise(modelURL16);
        for (num = 0; num < 2; num++) {

            arr4[num] = loadModelPromise(modelURL6);//无run用walk代替

        }
        arr4[2] = loadModelPromise(modelURL6);
        arr4[3] = loadModelPromise(modelURL9);
        arr4[4] = loadModelPromise(modelURL9);//无crawl用bend代替
        arr4[5] = loadModelPromise(modelURL14);
        ...
        var promiseAll2 = Promise.all(arr2).then((data) => {
            ...
            for (var i = 0; i < number; i++) {
                temp = i % 2;
                var newMesh, textureURL, textureURL1;
                newMesh = cloneGltfWithAllAnimation(data[temp],data[2],data[3],data[4],data[5]);
        ...
        var promiseAll4 = Promise.all(arr4).then((data) => {
            ...
            number = multi;
            for(var i=0;i<proportion.length-1;i++)
                number -= Math.round(multi*proportion[i]);
            for (var i = 0; i < number; i++) {
                temp = i % 2;
                var newMesh, textureURL;
                newMesh = cloneGltfWithAllAnimation(data[temp],data[2],data[3],data[4],data[5]);
